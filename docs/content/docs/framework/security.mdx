...
如果 MCP Gateway 还需要基于当前登录用户做资源鉴权，可以开启 TIP 传播：
...
开启后，`VeIdentityMcpToolset` 会在真实 `tools/call` 时从运行时 credential service 的 `inbound_auth` 读取用户池 access token / JWT，调用 Agent Identity 的 `get_workload_access_token_for_jwt` 交换 workload access token，并通过 `X-Ve-TIP-Token` 发送给 MCP Gateway。工具发现阶段的 `initialize` / `tools/list` 不携带 TIP token，只使用 `auth_config` 生成的服务级认证头。
